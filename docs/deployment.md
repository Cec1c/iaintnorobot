...
推荐在 AstrBot WebUI 的插件配置面板里先添加群聊。`managed_groups` 里启用的群都会作为目标群运行；不再需要单独选择“当前目标群”。如果是手动填写群号，插件会尽量用已观察到的平台自动补齐会话；补不出来时，让目标群里发一条普通消息，或在该群使用 `/iar addgroup`，即可完成初始化。
...
已添加群聊列表。插件只会在这里添加且启用的群聊中运行。每个群条目里的“是否在该群启用”就是该群的开关。后台触发按群号逐个计算；每个群需要有自己的会话记录和近期消息。
